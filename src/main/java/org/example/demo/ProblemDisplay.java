...
public class ProblemDisplay {
    ...
    static void drawInstance(Parent root, Stage stage) {
        ...
        for (CoordsPair coordsPair : coords) {
            if (minX > coordsPair.getLat())
                minX = coordsPair.getLat();
            if (minY > coordsPair.getLng())
                minY = coordsPair.getLng();
        }
        for (CoordsPair coordsPair : coords) {
            coordsPair.setLat(coordsPair.getLat() - minX);
            coordsPair.setLng(coordsPair.getLng() - minY);
            if (maxX < coordsPair.getLat())
                maxX = coordsPair.getLat();
            if (maxY < coordsPair.getLng())
                maxY = coordsPair.getLng();
        }
    ...
    private static void drawSolution(List<Integer> order) {
